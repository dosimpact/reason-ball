...
상태: 구현 기준. 요구사항은 [01](01-requirements.md), 공통 원칙은 [00](00-principles.md)을 따른다.

## 실행 구성

- Next.js + TypeScript, Node.js 단일 프로세스에서 UI·API·MCP·SSE를 제공한다.
- MCP는 `@modelcontextprotocol/sdk`의 stateless Streamable HTTP POST `/mcp`를 사용한다. 실행 설정의 원본은 [09](09-implementation.md)이다.
...
- Figma REST API 읽기는 서버 경계에서 수행한다. UI/API와 MCP가 같은 서비스를 사용한다. 토큰·조회 제한은 [10](10-completion.md)을 따른다.
...
- 상세 계약은 [08](08-mcp-storage-sse.md), 실행 기본값은 [09](09-implementation.md)를 따른다.

## 화면·생명주기

- 일곱 타입과 프로젝트 인덱스는 [03](03-document-catalog.md), API·Figma·Weblogging·DB는 [07](07-document-type-contracts.md)를 따른다.
- Flow Overview/Detail은 같은 JSON 트리·파서·렌더러를 사용한다. 문법과 탐색은 [04](04-flow-spec.md)·[05](05-flow-spec-tree.md)가 원본이다.
- SSE를 구독한 후 초기 조회하며 ready/재연결/변경 시 조회를 반복한다. 요청 세대로 오래된 응답을 무시한다. 15초 heartbeat와 종료 시 정리를 수행한다.
- 입력·사실·가정·질문·사용자 승인과 고정 버전 인계는 [06](06-design-lifecycle.md)을 따른다.
- 추가 문서 관계·비교·Flow 편집·Figma 수집은 [10](10-completion.md)을 따른다.
