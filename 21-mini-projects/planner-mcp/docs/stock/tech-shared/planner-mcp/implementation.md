...
- 협업 요약은 `src/app/server/collaboration.ts`에서 기존 저장소 조회를 조합하고 UI API `collaboration`으로 제공한다. `collaboration-hub.tsx`는 프로젝트 홈과 검토함을 담당한다. 저장소 스키마 변경은 검토 의견의 선택적 `questionId` 추가뿐이며 기존 데이터는 그대로 읽는다. 계약은 [11](../../project-design/11-collaboration-ux.md)을 따른다.
...
Figma 수집·문서 관계·버전 비교·Flow 노드 편집을 추가했습니다. 계약 원본은 [10](../../project-design/10-completion.md)입니다. 사용자 인증은 사용자 요청으로 제외합니다.
...
- 추가 MCP 도구: `import_figma`, `edit_flow_node`, `get_document_relations`, `compare_documents`. 기존 15개. 템플릿 5개를 포함해 현재 20개.
...
- `/templates`: 프로젝트 없이 사용하는 공용 템플릿·예시·AI 사용 프롬프트 관리.
- `src/app/server/templates.ts`는 기존 트랜잭션에 템플릿 저장·삭제를 조합한다. `entities/template`와 `widgets/template-manager`의 상세 계약은 [템플릿 설계](../../document-templates/system-design.md)를 따른다.
- `test:api`: 생산 빌드, 임시 데이터·임의 포트의 소유 서버, `e2e/bruno-api-tests` 컬렉션 실행과 정리.
- `test:e2e`: 위 API 검증 후 기존·신규 Playwright 회귀를 실행한다.
- `storybook`: `127.0.0.1:6006`에서 viewer story를 연다. `build-storybook`은 무시되는 storybook-static에 생성한다.
- `node scripts/e2e.mjs inspect`: 이미 생성된 생산 빌드를 임시 데이터·임의 포트에서 수동 브라우저 검증한다. SIGINT/SIGTERM 종료 시 소유 서버·데이터를 정리한다.
