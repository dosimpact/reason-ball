"""
Example 38 — Evaluator loop.

생성 노드가 답변을 만들고, evaluator 노드가 명시적인 품질 기준으로
PASS / FAIL 을 판단한 뒤 실패하면 재작성하는 패턴.

Reflection(36) 이 자유형 critique 중심이라면, 이 예제는 실무에서 더 자주 쓰는
"점수/통과 여부/재시도 상한" 중심 evaluator loop 를 보여줍니다.

선행 예제
---------
- 36: 자유형 critique 기반 reflection loop
- 10: Pydantic structured output

새 개념
-------
- PASS/FAIL, score, feedback으로 구조화된 evaluator
- 통과 여부와 최대 시도 횟수를 함께 적용하는 loop guard

복습 개념
-------
- structured output, conditional edge, cycle
