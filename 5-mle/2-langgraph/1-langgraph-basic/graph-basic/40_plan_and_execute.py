"""
Example 40 — Plan-and-Execute simulation.

복잡한 작업을 LLM 으로 **단계 리스트(plan)** 를 만들고, 각 단계를 순서대로 실행하면서
plan 을 줄여나가는 패턴.

여기의 executor는 외부 API나 tool을 실제로 수행하지 않고,
LLM에게 각 step의 결과 텍스트를 생성하게 하는 **실행 시뮬레이션**입니다.
실제 에이전트는 step 종류에 따라 tool/subgraph로 위임해야 합니다.

선행 예제
---------
- 10: structured output으로 구조화된 plan 생성
- 06: cycle과 종료 조건

새 개념
-------
- plan queue를 state에 보관하고 하나씩 소진하는 흐름

복습 개념
-------
- Pydantic structured output, conditional loop, iteration limit
