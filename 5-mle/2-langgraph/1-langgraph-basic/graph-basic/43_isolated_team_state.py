"""
Example 43 — Hierarchical Supervisor with per-team isolated state.

42는 부모 그래프와 모든 팀 subgraph가 **하나의 State 클래스**를 공유했다.
구현은 단순하지만 다음 단점이 있다:
...
이 내부 scratch state의 수명은 **subgraph를 한 번 진입해 종료할 때까지**다.
부모로 복귀한 후 같은 team에 다시 진입해도, 부모와 공유하지 않는
scratch key는 자동으로 복원되지 않는다. 재진입 간 보존이 필요하면
부모 state, checkpointer namespace, store 중 하나를 명시적으로 설계해야 한다.

선행 예제
---------
- 32: subgraph의 public input/output과 private internal state
- 42: hierarchical supervisor

새 개념
-------
- team별 서로 다른 state schema와 공유 channel
- private scratch state의 경계와 수명

복습 개념
-------
- parent/subgraph composition, structured routing
