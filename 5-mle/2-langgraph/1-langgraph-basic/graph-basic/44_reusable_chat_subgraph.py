"""
Example 44 — **state-routed reusable chat subgraph** 패턴.

43의 "부모/자식 schema 분리 + messages 만 공유" 패턴을 응용하여,
**하나의 compiled subgraph 를 여러 위치에 재부착**하면서, 각 호출 시점에
**부모가 control field와 payload를 주입**해 동작을 제어한다.

이 예제는 `langgraph.types.Command`를 사용하는 동적 goto 패턴이 아니다.
`chat_command`라는 일반 state field를 conditional edge가 읽어 분기하는 패턴이다.

선행 예제
---------
- 32: input/output/internal state schema
- 43: 부모와 subgraph의 state 경계

새 개념
-------
- 하나의 compiled subgraph를 여러 parent node 위치에 재사용
- `output_schema` 로 API 출력에서 내부 control/scratch field 제외
- `subgraphs=True` stream namespace로 특정 subgraph event를 식별

복습 개념
-------
- state-based routing, shared keys, input/output schema

Note
Q. API/UI에 필요한 output field만 노출하는게 가능?
...
2. 부모/자식 모두 `chat_command`, `chat_payload` 를 schema 에 두면, LangGraph 가
   키 일치 기반으로 자동 전달한다 (= 43의 messages 공유와 같은 메커니즘).
...
# ---------------------------------------------------------------------------
# chat control 상수 (`langgraph.types.Command`와 다름)
# ---------------------------------------------------------------------------
...
class ApiOutputState(MessagesState):
    """API/UI에 노출할 parent graph의 public output."""
...
if __name__ == "__main__":
    ...
    # 기대: final, messages, stage
    # analysis/chat_command/chat_payload/ui_step_count는 public output에 노출되지 않음.
