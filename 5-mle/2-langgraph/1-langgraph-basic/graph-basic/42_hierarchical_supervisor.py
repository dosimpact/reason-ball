"""
Example 42 — Hierarchical Supervisor (계층적 멀티 워커 라우팅).

41 번 예제(평면 supervisor)는 worker 가 늘어날수록
...
선행 예제
---------
- 41: 평면 supervisor와 worker 라우팅
- 31: compiled subgraph 조합

새 개념
-------
- top supervisor가 team을, team supervisor가 worker를 선택하는 2단계 라우팅
- `team_dispatches`, `completed_workers`로 prompt 규칙을 코드 guard로 보강

복습 개념
-------
- subgraph, structured routing, max-iteration guard
...
학습 포인트
-----------
- 컴파일된 subgraph를 부모의 노드로 부착 (예제 31과 동일 메커니즘)
...
class State(MessagesState):
    ...
    completed_workers: list[str]  # 현재 team dispatch에서 실행한 worker
    team_dispatches: dict[str, int]  # top level의 team별 실행 횟수
...
def _make_llm_worker(name: str, role_prompt: str):
    ...
    def node(state: State) -> dict:
        ...
        completed = [*state.get("completed_workers", []), name]
        return {
...
def _make_team_supervisor(team_name: str, workers: list[str], hint: str):
    ...
    def team_supervisor(state: State) -> dict:
        ...
        if iters > MAX_TEAM_ITERS or completed.issuperset(workers):
            return {
                "next_worker": "DONE",
                "team_iters": iters,
                "messages": [
        ...
        if next_worker in completed:
            # Prompt 규칙에만 의존하지 않고 중복 worker 실행을 코드로 차단.
            next_worker = "DONE"
            reason = f"duplicate worker blocked: {routed.next_worker}"
        return {
            "next_worker": next_worker,
            "team_iters": iters,
            "messages": [
                AIMessage(
                    content=f"[{team_name}_supervisor → {next_worker}] {reason}",
                    name=f"{team_name}_supervisor",
                )
            ],
        }
...
def _build_team_subgraph(team_name: str, workers: dict, hint: str):
    ...
    # top_supervisor가 팀 진입 전에 team_iters를 0으로 리셋한다.
    # subgraph 안에서는 현재 팀 실행에 대한 횟수만 누적한다.
...
def top_supervisor(state: State) -> dict:
    ...
    reason = routed.reason
    dispatches = dict(state.get("team_dispatches", {}))
    if next_team != "FINISH":
        if dispatches.get(next_team, 0) >= 2:
            next_team = "FINISH"
            reason = f"team dispatch limit reached: {routed.next_team}"
        else:
            dispatches[next_team] = dispatches.get(next_team, 0) + 1
    return {
        "next_team": next_team,
        "top_iters": iters,
        ...
        "team_iters": 0,
        "completed_workers": [],
        "team_dispatches": dispatches,
        "messages": [
            AIMessage(
                content=f"[top_supervisor → {next_team}] {reason}",
                name="top_supervisor",
            )
        ],
    }
