"""
Example 30 — Map-Reduce (Send API).
...
선행 예제
---------
- 29: 정적 parallel branch, join barrier, reducer

새 개념
-------
- `Send`로 런타임 개수만큼 worker를 만드는 동적 fan-out
- worker별 private input과 reducer를 이용한 fan-in

복습 개념
-------
- parallel branch가 모두 끝난 뒤 aggregate가 실행되는 join
...
- `from langgraph.types import Send` — 동적 fan-out
...
- {"topics": []}                                       ← worker 없이 aggregate로 이동, answer=""
"""
...
def fanout(state: State) -> list[Send]:
    ...
    if not topics:
        # Send가 하나도 없으면 후속 node도 실행되지 않는다.
        # 빈 입력도 일관된 output contract(answer: str)를 갖도록
        # aggregate를 명시적으로 한 번 스케줄한다.
        return [Send("aggregate", {})]
    return [Send("worker", {"topic": topic}) for topic in topics]
...
def build_graph():
    ...
    builder.add_conditional_edges("dispatch", fanout, ["worker", "aggregate"])
