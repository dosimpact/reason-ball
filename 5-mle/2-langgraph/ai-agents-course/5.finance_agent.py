"""
Finance performance reporting workflow with LangGraph.
Beginners: this file is organized as multiple "nodes" connected in a graph:
1) gather_financials -> 2) analyze_data -> 3) research_competitors
-> 4) compare_performance -> (feedback loop) -> write_report
"""
...
# Keep keys in .env instead of hardcoding values.
...
if tavily_api_key:
    ...
    print("[warn] TAVILY_API_KEY is not set. Web search nodes will return empty results.")
    tavily = _NoopTavily()
...
class AgentState(TypedDict):
    # Shared state object passed through the whole graph
    task: str
...
class Queries(BaseModel):
    # Used when model returns structured search queries
    queries: List[str]
...
def gather_financials_node(state: AgentState):
    """
    Node: parse CSV text, convert it to readable plain text,
    and ask LLM to extract key financial facts.
    """
...
def analyze_data_node(state: AgentState):
    # Node: ask LLM to analyze the extracted financial numbers.
    messages = [
...
def research_competitors_node(state: AgentState):
    # Node: for each competitor, ask model to generate research queries and collect web snippets.
...
def compare_performance_node(state: AgentState):
    # Node: build a competitor comparison report from analysis + gathered web content.
...
def research_critique_node(state: AgentState):
    # Node: if reviewer feedback exists, search more sources for those weak points.
    queries = model.with_structured_output(Queries).invoke(
        [
...
def collect_feedback_node(state: AgentState):
    # Node: generate review feedback on comparison output.
    messages = [
...
def write_report_node(state: AgentState):
    # Node: produce final markdown-like report text from comparison + feedback context.
    messages = [
...
def should_continue(state):
    # Stop loop after max revisions; otherwise go back to feedback node.
...
# Optional note:
# compare_performance -> collect_feedback -> research_critique -> compare_performance can repeat.
...
def main():
    # Streamlit entrypoint: UI for task/competitors/upload and run the graph.
    ...
    if st.button("Start Analysis") and uploaded_file is not None:
        ...
        try:
            # Stream each intermediate graph state so users can see progress.
            final_state = None
            for s in graph.stream(initial_state):
...
if __name__ == "__main__":
    # Run local Streamlit app when directly executing this file.
    main()

# ==== End Streamlit UI ====


# Note: to run this Streamlit app, make sure you have Streamlit installed and then execute:
# streamlit run 5.finance_agent.py 