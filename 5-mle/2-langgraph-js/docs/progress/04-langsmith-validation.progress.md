...
| VALIDATE-02 | Done | Codex + subagents | graph smoke tests | `pnpm --filter @reason-ball/langgraph-examples test` | 12 graph tests passed |
...
```bash
set -a; source .env; set +a; LANGSMITH_TRACING=true pnpm --filter @reason-ball/langgraph-examples exec tsx -e "import { graph } from './src/graph-basic/01-simple-graph.ts'; (async () => { const out = await graph.invoke({ text: 'langsmith trace check', steps: [] }, { tags: ['langsmith-validation','graph-01'], metadata: { task: 'VALIDATE-04', graphId: 'b_01_simple' } }); console.log(JSON.stringify(out)); })();"
```
