...
from langgraph_fast.domains.tenk.models import FilingDocument
...
def fact_key(subject: str, predicate: str, object_or_complement: str) -> str:
    return stable_hash(f"{normalize_key(subject)}|{normalize_key(predicate)}|{normalize_key(object_or_complement)}")
...
def _statement_contains(statement_text: str, probe: str) -> bool:
    ...
    return bool(normalized_probe) and normalized_probe in normalize_key(statement_text)
...
def _dedupe_graph_payload(
    ...
    for node in nodes:
        identity = (tuple(node.get("labels") or []), tuple(sorted((node.get("key") or {}).items())))
...
class GraphMapper:
    ...
    def map(self, extracted: Any, metadata: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        nodes: list[dict[str, Any]] = [
            {
                "labels": ["Company"],
                "key": {"id": company_id},
                "properties": {"name": document.company_name, "ticker": document.ticker, "cik": document.cik},
            },
            {
                "labels": ["Filing"],
                "key": {"id": filing_id},
        ...
        for section_row in payload.get("sections", []):
            ...
            section = _to_dict(row.get("section"))
            result = _to_dict(row.get("result"))
            item_id = item_key(filing_id, str(section.get("item_code") or "UNKNOWN"))
            sec_id = section_key(item_id, str(section.get("section_id") or "unknown"))
            nodes.extend(
                [
                    {
                        "labels": ["Item"],
                        ...
                        "properties": {
                            ...
                            "part_code": section.get("part_code"),
                            "filing_id": filing_id,
                        },
                    },
                    {
            ...
            for fact in result.get("facts", []):
                ...
                nodes.append({"labels": ["Fact"], "key": {"id": fact_id}, "properties": fact_dict})
                for statement_id, statement_dict in statement_entries:
                    text = str(statement_dict.get("text") or "")
                    if _statement_contains(text, str(fact_dict.get("subject") or "")):
                        relationships.append(
                            {
                                "type": "SUPPORTED_BY",
            ...
            for entity in result.get("entities", []):
                ...
                nodes.append({"labels": ["Entity"], "key": {"id": entity_id}, "properties": entity_dict})
                for statement_id, statement_dict in statement_entries:
                    if _statement_contains(str(statement_dict.get("text") or ""), str(entity_dict.get("value") or "")):
                        relationships.append(
                            {
                                "type": "MENTIONS",
                                "start": {"labels": ["Statement"], "key": {"id": statement_id}},
            ...
            for metric in result.get("metrics", []):
                ...
                nodes.append({"labels": ["Metric"], "key": {"id": metric_id}, "properties": metric_dict})
                relationships.append(
                    {
                        "type": "HAS_METRIC",
                        "start": {"labels": ["Item"], "key": {"id": item_id}},
            ...
            for risk in result.get("risks", []):
                ...
                risk_id = risk_key(item_id, str(risk_dict.get("risk_type") or ""), str(risk_dict.get("risk_text") or ""))
                nodes.append({"labels": ["Risk"], "key": {"id": risk_id}, "properties": risk_dict})
                relationships.append(
                    {
                        "type": "HAS_RISK",
                        "start": {"labels": ["Item"], "key": {"id": item_id}},
