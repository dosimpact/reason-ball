...
이 폴더는 `1-reason-hwang` A2UI 기술 내용의 현재 원본이다. 공통 개념부터 패키지 구현, 도메인 적용, 운영 순서로 읽는다. 기존 기능 상태는 **OAuth 검증 범위 구현·검증 완료**이다. SEC 재무 차트 확장의 구현·검증 상태는 전용 문서를 따른다. 과거 결정·실행 증거는 `docs/flow/`에 보존한다.

| 문서 | 내용 |
| --- | --- |
| [시스템 설계](a2ui-system.md) | 요구사항, Dynamic/Fixed 차이, 전체 구조, 고정 버전, 신뢰 경계 |
| [Registry와 카탈로그](registry-and-catalog.md) | 61개 UI 파일/67개 어댑터 목록, 4개 하위 카탈로그, 생성·확장 절차 |
...
| [SEC 재무 차트 설계](sec-financial-charts.md) | 목차 선택 읽기, 데이터 추출/차트 렌더 두 도구, 6종 차트, 수치·출처 검증 |
