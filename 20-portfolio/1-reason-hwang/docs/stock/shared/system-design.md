...
The SEC surface includes company synchronization and listing, filing synchronization/backfill/download/retry jobs, filing status and report queries, and parser-status updates. PostgreSQL access is implemented with TypeORM entities and migrations. Swagger is enabled by default at `/docs/sec`, with JSON and YAML documents below that path; set `SWAGGER_ENABLED=false` to disable it.
...
When PostgreSQL is configured, the application persists assistant, thread, run, cron, store, A2A, and LangGraph checkpoint data. Local profile startup may prepare idempotent PostgreSQL schema changes and migrate legacy snapshots. FastAPI startup also prepares the 10-K Neo4j uniqueness constraints in the local profile. For both databases, non-local profiles verify the expected schema without executing DDL and fail fast when it is absent or outdated. The supported FastAPI deployment is one Uvicorn worker because distributed run coordination is outside the service boundary.
...
- SEC contract: `2-bff-apps/src/sec/API_SPEC.md`.
...
- Historical and focused investigations: `master-docs/`.
