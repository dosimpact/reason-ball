...
Detailed procedures and completion/evidence rules are canonical in [Validation Principles](../../validation/README.md). Read them before selecting checks. Unit tests, lint, type checks, builds, and CLI E2E alone do not replace the required methods. Missing environments or tools must be reported as incomplete validation; existing opt-in database test policy does not waive required API E2E.
...
- Filing sync, download, and retry jobs.
- Filing status, listing, downloaded reports, and parser-status updates.
