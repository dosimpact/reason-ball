...
def _extract_callback_code(callback_url: str, expected_state: str) -> str:
    """Extract and validate the OAuth authorization code from a callback URL."""
    parsed = urlparse(callback_url.strip())
    qs = parse_qs(parsed.query)

    received_state = qs.get("state", [""])[0]
    if received_state != expected_state:
        raise RuntimeError("OAuth state mismatch. Restart login and paste the latest callback URL.")
...
async def login(force: bool = False, manual_callback: bool = False) -> dict:
    ...
    params = {
        ...
        "originator": "chatgpt_oauth_proxy",
    }
    ...
    if manual_callback:
        ...
        print("After login, copy the full localhost callback URL from the browser address bar.")
        callback_url = input("Callback URL: ").strip()
        code = _extract_callback_code(callback_url, state)
        logger.info("Authorization code received from pasted callback URL")
    else:
...
def _main() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m core.oauth_login",
    ...
    parser.add_argument(
        "--manual-callback",
        action="store_true",
        help="Paste the localhost callback URL manually instead of running a callback server.",
    )
    ...
    asyncio.run(login(force=args.force, manual_callback=args.manual_callback))
