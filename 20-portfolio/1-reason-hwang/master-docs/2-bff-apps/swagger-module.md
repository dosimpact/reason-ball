...
Swagger 문서화는 런타임 입력 검증을 대신하지 않는다. 요청 검증이 필요하면 `class-validator`, `class-transformer`, `ValidationPipe`를 별도로 구성해야 한다.
...
| `src/sec/api/collector.controller.ts` | 엔드포인트별 설명, 쿼리, 본문, 응답 선언 |
| `src/sec/api/dto/swagger.dto.ts` | 요청·응답 스키마와 예시 선언 |
...
명시적으로 작성한 `@ApiProperty()` 옵션은 자동 생성값보다 우선한다. 따라서 현재 `swagger.dto.ts`처럼 예시와 도메인 설명이 중요한 DTO는 수동 데코레이터를 유지하고, 단순 DTO는 플러그인 추론에 맡기는 혼합 방식이 적절하다.
