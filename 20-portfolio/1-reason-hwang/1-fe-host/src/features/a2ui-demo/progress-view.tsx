export const labels = {
  financial_index: "보고서 목차를 확인하고 있습니다",
  financial_read: "선택한 재무 표를 읽고 있습니다",
  financial_validate: "원문 수치와 단위를 검증하고 있습니다",
  financial_compose: "검증된 데이터로 차트를 구성하고 있습니다",
...
export function ProgressView({ progress, compact = false }: { progress: ProgressState | null; compact?: boolean }) {
  ...
  if (compact) return <p role="status" aria-live="polite" className="mb-3 text-sm text-muted-foreground">{progress.status === "running" ? (progress.stages.length ? labels[progress.stages.at(-1)!] : "요청을 처리하고 있습니다. 분석에는 시간이 걸릴 수 있습니다.") : title}</p>;
