...
function DemoSession({ mode }: { mode: "dynamic" | "fixed" | "sec" }) {
  ...
  return <CopilotKitProvider runtimeUrl={`/api/copilotkit/a2ui/${mode}`} agentId={`a2ui-${mode}`} useSingleEndpoint properties={properties} a2ui={{ catalog, includeSchema: false }} renderActivityMessages={renderers} enableInspector={false} onError={({ error }) => setError(error.message)}>
    ...
    <div className="mb-4 rounded-lg bg-muted p-4 text-sm">{mode === "sec" ? "회사명 또는 티커를 입력하세요. 예시: AAPL · Apple · Microsoft. 공시를 선택한 뒤 보고서 생성을 누르세요." : mode === "dynamic" ? "예시: 전체 매출 현황을 보여줘 · 담당자별 실적을 비교해줘 · 지역별 매출 비중을 보여줘" : fixedDemoCopy.examples}</div>
    ...
    <div className="min-h-[60vh] rounded-xl border"><CopilotChat agentId={`a2ui-${mode}`} labels={{ welcomeMessageText: mode === "sec" ? "어떤 회사를 조회할까요? 회사명 또는 티커를 입력하세요." : mode === "dynamic" ? "어떤 매출 정보를 살펴볼까요?" : fixedDemoCopy.welcome, chatInputPlaceholder: mode === "sec" ? "회사명 또는 티커" : "질문을 입력하세요" }} /></div>
  </CopilotKitProvider>;
}
