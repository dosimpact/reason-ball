...
이 프로젝트의 설계 문서는 [`docs/README.md`](docs/README.md)에 정의된 저량(Stock)·유량(Flow) 체계로 관리한다.

### 저량 — 현재 상태의 단일 기준

- [`docs/stock/business-design.md`](docs/stock/business-design.md): 최신 비즈니스 문제, 사용자, 기능 범위, 요구사항과 완료 기준
- [`docs/stock/system-design.md`](docs/stock/system-design.md): 최신 시스템 구조, 데이터·보안 경계, 구현 원칙과 운영 제약
- [`docs/stock/test-design.md`](docs/stock/test-design.md): 최신 테스트 전략, 요구사항 검증 상태와 미완료 release gate

에이전트는 중요한 변경을 계획하거나 구현하기 전에 관련 저량 문서를 읽는다. 현재 제품이나 시스템을 설명할 때는 날짜별 로그를 조합해 추정하지 말고 저량 문서를 기준으로 삼는다.

### 유량 — 변경 과정과 시점별 근거

`docs/flow/YYYY-MM-DD-<topic>.md`에 요구사항 변경, 설계 결정, migration, 구현 진행, 감사와 검증 결과를 기록한다. 유량 기록에는 날짜, 배경, 변경 내용, 이유, 영향받는 요구사항·저량 절, 실행한 검증, 결과와 남은 작업을 포함한다.

- 기존 유량 기록은 당시의 사실이므로 최신 상태에 맞춰 고쳐 쓰거나 삭제하지 않는다.
- 과거 판단이 바뀌면 새 유량 기록에서 정정하고 대체 근거를 남긴다.
- mock, 계약 검사, PGlite, 실제 Supabase와 실제 AI 검증을 구분하며 부분 실행을 전체 PASS로 표현하지 않는다.

### 변경 완료 조건

1. 작업 전에 관련 저량 문서와 최근 유량 기록을 확인한다.
2. 작업 중 또는 검증 직후 날짜별 유량 기록을 추가하거나 갱신한다.
3. 확정된 현재 상태를 비즈니스·시스템·테스트 저량 문서 중 영향받는 모든 문서에 반영한다.
4. 요구사항 ID와 설계 결정, 코드, migration, 테스트 및 검증 근거를 가능한 한 상호 연결한다.
5. 코드·저량·유량이 다르면 구현과 승인된 결정을 확인하고 저량을 바로잡은 뒤 조정 내용을 새 유량 기록에 남긴다.
6. 저량과 유량의 동기화가 끝나기 전에는 문서가 필요한 기능 변경을 완료로 보고하지 않는다.

`AGENTS.md`는 에이전트 작업 규칙과 반드시 지켜야 할 로컬 제약을 담는다. 제품·시스템의 상세 현재 상태를 이 파일에만 추가하지 말고 해당 저량 문서에도 반영한다.
...
이 프로젝트의 신규 코드와 수정하는 코드에는 [개발 설계서의 SLAP·순수함수 지침](docs/stock/system-design.md#54-slap과-순수함수)을 적용한다. 기존 코드 전체가 이 기준을 충족한다고 가정하지 않는다.
