...
test("guest saves private preferences to Supabase and restores them after reload", async ({ page }) => {
...
test("guest saves and removes a real mission across reloads", async ({ page }) => {
...
test("mission chat persists the user and assistant turns and restores the same conversation", async ({ page }) => {
  ...
  await expect(page.getByTestId("message-assistant").last()).toContainText("What real-life situation would you like to practice today?");
  // The model output is deterministic, but these rows must come from the real DB.
  await expect.poll(async () => {
    const rows = await adminClient().from("messages").select("role, status").eq("conversation_id", conversationId);
    expect(rows.error).toBeNull();
    return rows.data?.filter((row) => row.status === "complete" && ["user", "assistant"].includes(row.role)).length;
  }).toBe(2);
  await page.reload();
