...
export async function loadAssistanceMessages(client: SupabaseClient, ownerId: string, input: AssistanceRequest) {
  ...
  const found = await client.from("messages").select("id,client_message_id,role,parts,sequence_number").eq("conversation_id", input.conversationId).eq("status", "complete")
  ...
  const history = await client.from("messages").select("id,role,parts,sequence_number").eq("conversation_id", input.conversationId).eq("status", "complete").in("role", ["user", "assistant"])
