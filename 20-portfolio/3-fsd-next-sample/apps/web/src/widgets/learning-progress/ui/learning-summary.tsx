...
export function LearningSummary({ xp }: { xp?: number }) {
  ...
  return <section aria-label="나의 학습 요약" data-testid="home-learning-summary">
    ...
    ].map((stat) => <article key={stat.label} className="rounded-2xl border border-border bg-card text-card-foreground p-5">
      <h2 className="text-sm font-bold text-muted-foreground">{stat.label}</h2><p className="mt-2 text-2xl font-black">{stat.value}</p><p className="mt-2 text-xs text-muted-foreground">{stat.detail}</p>
    </article>)}</div>
  </section>;
}
