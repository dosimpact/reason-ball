...
import { MissionHints } from "@/features/mission-hint/ui/mission-hints";

export function MissionGuidancePanel({ guidance, runId, open, onOpenChange, onInsert, onRetry, disabled }: {
  ...
  return <section className="mb-2 rounded-xl border border-indigo-100 bg-indigo-50/60 text-xs" data-testid="mission-guidance">
    ...
    {open ? <div id={id} className="max-h-[min(22rem,40svh)] space-y-2 overflow-y-auto border-t border-indigo-100 p-3">
      {guidance.state === "loading" ? <p role="status">미션 실행 정보를 기다리고 있어요.</p> : guidance.state === "unavailable" ? <div role="alert"><p>실행 단계와 힌트를 연결하지 못했어요. 다른 단계의 힌트로 대체하지 않았습니다.</p><button type="button" disabled={disabled} onClick={onRetry} className="mt-2 underline">단계 정보 다시 불러오기</button></div> : <>
        ...
        {selected ? <><p className="font-bold">{previewId ? "선택한 연습 단계" : "현재 실행 단계"}: {selected.label}</p>
          {runId ? <MissionHints key={`${runId}:${selected.id}`} runId={runId} stepId={selected.id} disabled={disabled} onInsert={onInsert} /> : <><p className="whitespace-pre-wrap break-words" data-testid="mission-step-hint">{selected.hint || "이 단계에는 작성자가 제공한 힌트가 없어요."}</p>
        ...
        <p className="text-neutral-600">{runId ? "단계를 선택하거나 패널을 여는 것만으로 힌트를 요청하거나 목표를 완료하지 않습니다." : "단계 선택과 힌트 사용은 목표 완료나 보상에 영향을 주지 않습니다. 작성자 힌트이므로 상황에 맞게 고쳐 말해 보세요."}</p>
      </>}
    </div> : null}
  </section>;
}
