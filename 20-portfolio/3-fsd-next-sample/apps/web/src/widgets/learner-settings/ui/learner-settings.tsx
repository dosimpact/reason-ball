...
function SettingsForm({ initial, save, reload, remote }: {
  ...
  return <section className="mt-8 max-w-2xl" data-testid="profile-settings">
    ...
    <fieldset disabled={busy} className="mt-5 space-y-5 rounded-[1.5rem] border border-black/7 bg-white p-6 disabled:opacity-70">
      ...
      <label className="block text-sm font-bold">한국어 설명 양<select aria-label="한국어 설명 양" className="form-field" value={draft.koreanExplanation} onChange={(event) => change("koreanExplanation", event.target.value as LearningPreferences["koreanExplanation"])}><option value="none">영어만</option><option value="brief">짧게</option><option value="detailed">자세히</option></select></label>
      <label className="block text-sm font-bold">답변 길이<select aria-label="답변 길이" className="form-field" value={draft.responseLength} onChange={(event) => change("responseLength", event.target.value as LearningPreferences["responseLength"])}><option value="short">짧게</option><option value="standard">보통</option><option value="long">길게</option></select></label>
