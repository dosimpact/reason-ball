...
import { ArtifactAssistancePanel } from "./artifact-assistance";
...
export function ArtifactWorkspace({ artifacts, initialKind, onChange, onClose, persistence }: ArtifactWorkspaceProps) {
  ...
  async function copyCsv() {
    try { await copyText(draft); setCopyNotice("CSV를 복사했어요."); }
    catch { setCopyNotice("CSV를 복사하지 못했어요. 내용을 선택해 직접 복사해 주세요."); }
  }
    ...
    <div className="flex max-h-[94svh] w-full max-w-6xl flex-col overflow-hidden rounded-[1.7rem] bg-white shadow-2xl">
      ...
      <div className="grid min-h-0 min-w-0 flex-1 grid-rows-[auto_minmax(0,1fr)] overflow-hidden md:grid-cols-[240px_1fr] md:grid-rows-[minmax(0,1fr)]">
        ...
        <fieldset disabled={busy || imageStatus === "loading"} aria-label="Artifact 편집" className="min-h-0 min-w-0 overflow-y-auto p-5">{selected ? <div>
          ...
          {!persistence ? <div className="mt-2 flex flex-wrap gap-2"><button type="button" onClick={rewriteSelection} disabled={!draft} className="inline-flex items-center gap-2 rounded-lg border border-black/10 px-3 py-2 text-xs font-bold disabled:opacity-35"><WandSparkles className="size-3.5" /> 선택 영역 다듬기</button>{selected.kind === "text" ? <button type="button" onClick={applyGrammarSuggestion} className="inline-flex items-center gap-2 rounded-lg bg-indigo-50 px-3 py-2 text-xs font-bold text-indigo-700"><Sparkles className="size-3.5" /> 문법 제안 적용</button> : null}</div> : null}


          {persistence && selected.kind !== "image" ? <ArtifactAssistancePanel key={selected.id} id={selected.id} versionId={selected.versions.at(-1)!.id} kind={selected.kind} draft={draft} ready={saveStatus === "saved" && !busy} getSelection={() => ({ start: editorRef.current?.selectionStart ?? 0, end: editorRef.current?.selectionEnd ?? 0 })} onApply={async (content, expectedVersion) => {
            if (artifactsRef.current.find(item => item.id === selected.id)?.versions.at(-1)?.id !== expectedVersion || savingRef.current) return false;
            return persistVersion(selected.id, { title: title.trim() || selected.title, content, imageUrl }, true);
          }} /> : null}

          {selected.kind === "sheet" ? <section className="mt-4 rounded-xl border border-black/8 p-3" aria-label="Sheet 편집"><div className="overflow-x-auto"><table className="w-full border-collapse text-xs"><tbody>{sheetRows.map((row, rowIndex) => <tr key={rowIndex}>{row.map((cell, cellIndex) => <td key={cellIndex} className="border border-black/10 p-1"><input value={cell} onChange={(event) => updateCell(rowIndex, cellIndex, event.target.value)} className={`w-full min-w-28 bg-transparent px-1 py-1 outline-none ${rowIndex === 0 ? "font-bold" : ""}`} aria-label={`셀 ${rowIndex + 1}-${cellIndex + 1}`} /></td>)}</tr>)}</tbody></table></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={cleanSheet} className="rounded-lg border border-black/10 px-3 py-2 text-xs font-bold">데이터 정리</button><button type="button" onClick={analyzeSheet} className="rounded-lg border border-black/10 px-3 py-2 text-xs font-bold">{persistence ? "표 크기 확인" : "Sheet 분석"}</button><button type="button" onClick={() => void copyCsv()} className="rounded-lg border border-black/10 px-3 py-2 text-xs font-bold">CSV 복사</button><a href={`data:text/csv;charset=utf-8,${encodeURIComponent(draft)}`} download={`${title || "artifact"}.csv`} className="inline-flex items-center gap-1 rounded-lg bg-neutral-950 px-3 py-2 text-xs font-bold text-white" aria-label="CSV 다운로드"><Download className="size-3.5" /> CSV 다운로드</a></div>{copyNotice ? <p role="status" className="mt-2 text-xs">{copyNotice}</p> : null}{sheetAnalysis ? <p className="mt-3 rounded-lg bg-indigo-50 p-3 text-xs font-semibold text-indigo-700" data-testid="sheet-analysis">{sheetAnalysis}</p> : null}</section> : null}
