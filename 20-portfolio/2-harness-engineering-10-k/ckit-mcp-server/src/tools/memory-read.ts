...
const MEMORY_FILE = 'docs/.ckit-memory.json';
...
/**
 * ckit_memory_read - Read from session memory.
 */
  ...
  if (!memory) {
    if (await fileExists(memoryPath)) {
      try {
        ...
        console.error(`[ckit] Failed to read memory: ${err.message}`);
...
const definition = {
  name: 'ckit_memory_read',
  description: 'Read from ckit session memory (docs/.ckit-memory.json). Read a specific key or get all memory.',
  inputSchema: {
    type: 'object',
    properties: {
      key: {
