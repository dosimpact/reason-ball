...
describe('pdca/automation', () => {
  ...
  it('generatePdcaGuidance should provide phase-specific messages', async () => {
    ...
    assert.ok(g3.includes('ckit_pre_write_check'));
    assert.ok(g4.includes('Fix identified gaps'));
  });
});

describe('pdca/template', () => {
  it('selectTemplate should map plan/design/analysis/report/do templates by phase', () => {
  ...
  it('selectTemplate should fallback for unknown phase', () => {
    assert.strictEqual(selectTemplate('unknown'), 'plan.template.md');
  });
  ...
  it('resolveTemplateVariables should resolve placeholders and repeated keys', () => {
    const tmpl = 'A:${FEATURE} B:${DATE} D:${FEATURE}';
    const out = resolveTemplateVariables(tmpl, { FEATURE: 'f', DATE: '2026-02-15' });
    assert.strictEqual(out, 'A:f B:2026-02-15 D:f');
  });
  ...
  it('getTemplateList should return all 5 templates', () => {
    const list = getTemplateList();
    assert.strictEqual(list.length, 5);
    assert.ok(list.includes('design.template.md'));
  });
