...
describe('MCP Server', () => {
  it('should respond to initialize with protocol version and server info', async () => {
    ...
    assert.strictEqual(response.result.serverInfo.name, 'ckit-mcp');
    assert.ok(response.result.capabilities);
    assert.ok(response.result.capabilities.tools);
  });

  it('should return 13 tools from tools/list', async () => {
    const server = createServer();
    const response = await server.handleRequest({
      jsonrpc: '2.0',
      id: 2,
    ...
    assert.strictEqual(response.result.tools.length, 13);
  ...
  it('should handle tools/call with ckit_init', async () => {
    const server = createServer();
    const response = await server.handleRequest({
      jsonrpc: '2.0',
      id: 3,
