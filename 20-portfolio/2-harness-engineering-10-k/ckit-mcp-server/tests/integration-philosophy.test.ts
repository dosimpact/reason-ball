...
function mkProject() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ckit-int-'));
}
...
describe('integration and e2e', () => {
  ...
  it('tools/list should return exactly 13 tools', async () => {
    const server = createServer();
  ...
  it('full workflow init->plan->design->complete should maintain state', async () => {
    ...
    await server.handleRequest({
      ...
      id: 10,
      method: 'tools/call',
      params: { name: 'ckit_init', arguments: { projectDir } }
    });
    ...
    await server.handleRequest({
      ...
      id: 11,
      method: 'tools/call',
      params: { name: 'ckit_pdca_plan', arguments: { feature: 'auth' } }
    });
    ...
    await server.handleRequest({
      ...
      id: 12,
      method: 'tools/call',
      params: { name: 'ckit_pdca_design', arguments: { feature: 'auth' } }
    });
    ...
    const completed = await server.handleRequest({
      ...
      id: 13,
      method: 'tools/call',
      params: { name: 'ckit_complete_phase', arguments: { feature: 'auth', phase: 'design' } }
    });
  ...
  it('stdio entry should parse newline-delimited requests and return responses', async () => {
    ...
    try {
      ...
      assert.strictEqual(listRes.result.tools.length, 13);
    } finally {
      proc.stdin.end();
      proc.kill();
    }
...
describe('philosophy compliance checks', () => {
  ...
  it('Context Engineering: config should merge project config over defaults', async () => {
    const configPath = path.join(projectDir, '.ckit-codex/ckit.config.json');
  ...
  it('AI-Native: server should expose analysis tool for verification', () => {
    ...
    assert.ok(tools.some(t => t.name === 'ckit_pdca_analyze'));
  });
