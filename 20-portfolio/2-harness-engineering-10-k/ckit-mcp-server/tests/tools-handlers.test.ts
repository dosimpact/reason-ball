...
function mkProject() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ckit-tools-'));
...
describe('tool handlers', () => {
  ...
  it('ckit_init should return error when projectDir missing', async () => {
    const out = await initTool({}, context);
    assert.ok(out.error);
  });

  it('ckit_init should initialize context and return session info', async () => {
    const out = await initTool({ projectDir }, context);
    assert.strictEqual(context.projectDir, projectDir);
    assert.ok(out.sessionId.startsWith('ckit-'));
    assert.ok(out.pdcaStatus);
  });

  it('ckit_get_status should error before init', async () => {
    const out = await getStatusTool({}, { projectDir: null });
    assert.ok(out.error);
  });

  it('ckit_get_status should return all active features', async () => {
  ...
  it('ckit_get_status should return suggestion for unknown feature', async () => {
    await initTool({ projectDir }, context);
  ...
  it('ckit_pre_write_check should error when not initialized or missing filePath', async () => {
    assert.ok((await preWriteTool({ filePath: 'x' }, { projectDir: null })).error);
    await initTool({ projectDir }, context);
    assert.ok((await preWriteTool({}, context)).error);
  });

  it('ckit_pre_write_check should detect feature from path and return conventions', async () => {
    await initTool({ projectDir }, context);
  ...
  it('ckit_post_write should error when not initialized or missing filePath', async () => {
    assert.ok((await postWriteTool({ filePath: 'x' }, { projectDir: null })).error);
    await initTool({ projectDir }, context);
    assert.ok((await postWriteTool({}, context)).error);
  });

  it('ckit_post_write should suggest gap analysis for significant changes', async () => {
    await initTool({ projectDir }, context);
    await seedDocs(projectDir, 'auth');
    const filePath = path.join(projectDir, 'src', 'auth', 'service.js');
    fs.mkdirSync(path.dirname(filePath), { recursive: true });
  ...
  it('ckit_post_write should include large change recommendation for 200+ lines', async () => {
    await initTool({ projectDir }, context);
  ...
  it('ckit_complete_phase should validate input and invalid phase', async () => {
    await initTool({ projectDir }, context);
  ...
  it('ckit_complete_phase should complete phase and return next phase', async () => {
    await initTool({ projectDir }, context);
  ...
  it('ckit_complete_phase should block invalid transition', async () => {
    await initTool({ projectDir }, context);
    await planTool({ feature: 'feat' }, context);
  ...
  it('ckit_complete_phase should mark completed at report', async () => {
    await initTool({ projectDir }, context);
    await planTool({ feature: 'feat' }, context);
  ...
  it('ckit_pdca_plan should require session/feature and register feature', async () => {
  ...
  it('ckit_pdca_design should require plan and then return design template', async () => {
    await initTool({ projectDir }, context);
  ...
  it('ckit_pdca_analyze should require design and increment iteration', async () => {
    await initTool({ projectDir }, context);
    await planTool({ feature: 'my-feature' }, context);
  ...
  it('ckit_pdca_next should handle untracked and tracked feature', async () => {
    await initTool({ projectDir }, context);
  ...
  it('ckit_analyze_prompt should validate and return analysis', async () => {
  ...
  it('ckit_analyze_prompt should return clarifying questions for ambiguous prompt', async () => {
    await initTool({ projectDir }, context);
  ...
  it('ckit_select_template should validate phase and return template info', async () => {
  ...
  it('ckit_memory_read/write should validate, persist, and retrieve values', async () => {
    await initTool({ projectDir }, context);
