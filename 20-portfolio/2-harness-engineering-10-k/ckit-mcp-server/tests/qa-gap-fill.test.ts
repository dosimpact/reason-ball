...
function mkProject() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'ckit-gap-'));
}
...
describe('qa gap fill', () => {
  ...
  it('tool list should contain 13 unique names', () => {
    const tools = getToolDefinitions();
    assert.strictEqual(tools.length, 13);
    assert.strictEqual(new Set(tools.map(t => t.name)).size, 13);
  });

  it('template selection should support all phases', () => {
  ...
  it('template list should include required 5 files', () => {
