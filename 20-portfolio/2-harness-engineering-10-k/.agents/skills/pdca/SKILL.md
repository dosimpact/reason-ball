...
4. Call `ckit_pre_write_check(filePath)` before each file write
5. Call `ckit_post_write(filePath, linesChanged)` after significant changes
6. Call `ckit_complete_phase(feature, "do")` when done
...
1. Call `ckit_pdca_analyze(feature)` MCP tool
...
5. Call `ckit_complete_phase(feature, "act")` when done
...
1. Call `ckit_get_status(feature)` MCP tool
...
1. Call `ckit_pdca_next(feature)` MCP tool
